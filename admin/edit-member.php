...
if ($id <= 0) {
  ...
  echo "<h1 style='color:white;text-align:center;margin-top:3rem;'>Invalid member ID.</h1>";
  exit;
}
...
if (!$member) {
  ...
  echo "<h1 style='color:white;text-align:center;margin-top:3rem;'>Member not found.</h1>";
  exit;
}

// --- Fetch all departments from BOTH department_pricing + members ---
$stmt = $pdo->query("
...
document.getElementById('editForm').addEventListener('submit', async (e)=>{
  ...
  try {
    ...
    if (out.ok) {
      ...
      msgErr.textContent = '❌ ' + (out.error || 'Save failed.');
