...
/* Left Main Panel */
.main { flex:2.3; display:flex; flex-direction:column; padding:1.5rem 2rem; overflow:hidden; }
h1 {
  color:var(--brand);
  font-size:1.6rem;
  text-align:center;
...
p.subtitle { text-align:center; color:var(--gray); margin-bottom:1.5rem; }
...
.filters button.active { background:var(--brand); }
...
<body>
  <div class="main">
    ...
    <div class="filters">
      ...
      <button onclick="window.location='departments.php'">Departments</button>
    </div>
    ...
    <div class="table">
      <table id="members-table">
        <thead>
          <tr>
            <th>ID</th><th>Name</th><th>Department</th><th>Payment</th><th>Monthly Fee</th><th>Valid Until</th>
          </tr>
        </thead>
        <tbody>
...
function renderMembers(list){
  ...
  tableBody.innerHTML=list.map(m=>`
    <tr onclick="openDetail(${m.id})">
      ...
      <td>$${parseFloat(m.monthly_fee||0).toFixed(2)}</td>
      <td>${m.valid_until||'-'}</td>
    </tr>`).join('');
}
...
async function openDetail(id){
  ...
  try{
    ...
    detailContent.innerHTML=html+`<div style="margin-top:1rem;">
      <a class="btn-edit" href="edit-member.php?id=${id}">✏️ Edit Member</a>
