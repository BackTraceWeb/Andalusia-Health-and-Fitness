...
if ($requireHttps && (empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] === 'off')) { http_response_code(400); exit('HTTPS required'); }
...
if (!is_file($csvPath)) exit;
$fh = fopen($csvPath, 'rb'); if (!$fh) { http_response_code(500); exit('cannot open'); }
flock($fh, LOCK_SH); fpassthru($fh); flock($fh, LOCK_UN); fclose($fh);
