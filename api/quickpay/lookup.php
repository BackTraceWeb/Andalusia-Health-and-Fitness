...
try {
    ...
    file_put_contents($logFile, "Connected to DB: {$dbName} as {$dbUser}\n", FILE_APPEND);

    // Optional debug mode
    if (isset($_GET['debug'])) {
        echo json_encode(['debug_db' => $dbName, 'debug_user' => $dbUser]);
        exit;
    }
    ...
    if (!$candidates) {
        file_put_contents($logFile, "No members found for {$first} {$last}\n", FILE_APPEND);
